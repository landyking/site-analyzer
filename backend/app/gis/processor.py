    ...
    try:
        ...
        for cf in constraint_factors:
            try:
                ...
                restricted.append(
                    EngineRestrictedFactor(kind=kind, buffer_distance=buffer_distance)
                )
            except Exception as e:
        ...
        if task_out is not None:
            try:
                if os.path.exists(task_out):
                    ...
                    logger.info(
                        f"Temporary out directory '{task_out}' has been removed successfully"
                    )
                else:
...
def build_ranges(breakpoints: list[float], points: list[int]) -> list[tuple[float, float, int]]:
    ...
    return [tuple([intervals[i], intervals[i + 1], points[i]]) for i in range(len(points))]
