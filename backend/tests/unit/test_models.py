...
class TestDistrictHistogram:
    ...
    def test_district_histogram_creation(self):
        ...
        histogram = DistrictHistogram(
            frequency=[10, 20, 30], edges=[0.0, 1.0, 2.0, 3.0], min=0.0, max=3.0
        )
    ...
    def test_district_histogram_item(self):
        ...
        histogram = DistrictHistogram(frequency=[5, 15], edges=[0.0, 1.0, 2.0], min=0.0, max=2.0)
...
class TestAuthModels:
    ...
    def test_register_request_valid_data(self):
        ...
        register = RegisterRequest(email="newuser@example.com", password="securepass123")
...
class TestSuitabilityFactor:
    ...
    def test_suitability_factor_weight_validation(self):
        ...
        # Valid weights
        SuitabilityFactor(kind="slope", weight=0.1, breakpoints=[5.0], points=[10, 0])
        SuitabilityFactor(kind="slope", weight=10.0, breakpoints=[5.0], points=[10, 0])

        # Invalid weights
        with pytest.raises(ValidationError) as exc_info:
            SuitabilityFactor(kind="slope", weight=0.0, breakpoints=[5.0], points=[10, 0])
        assert "weight must be in the interval (0, 10]" in str(exc_info.value)

        with pytest.raises(ValidationError) as exc_info:
            SuitabilityFactor(kind="slope", weight=11.0, breakpoints=[5.0], points=[10, 0])
        assert "weight must be in the interval (0, 10]" in str(exc_info.value)
    ...
    def test_suitability_factor_empty_breakpoints_rejected(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            SuitabilityFactor(kind="slope", weight=5.0, breakpoints=[], points=[5])
    ...
    def test_suitability_factor_invalid_kind_rejected(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            SuitabilityFactor(
                kind="invalid_suitability", weight=5.0, breakpoints=[10.0], points=[8, 5]
            )
...
class TestMapTaskModels:
    ...
    def test_map_task_details_with_factors(self):
        ...
        suitability = SuitabilityFactor(kind="slope", weight=5.0, breakpoints=[10.0], points=[8, 2])
...
class TestCreateMapTaskReq:
    ...
    def test_create_map_task_req_valid_data(self):
        ...
        suitability = SuitabilityFactor(kind="slope", weight=3.0, breakpoints=[15.0], points=[9, 1])
    ...
    def test_create_map_task_req_invalid_district_code(self):
        ...
        suitability = SuitabilityFactor(kind="roads", weight=2.0, breakpoints=[10.0], points=[7, 3])
...
class TestAdminModels:
    ...
    def test_admin_update_user_status_request(self):
        ...
        request = AdminUpdateUserStatusRequest(user_id=456, status=UserStatus.LOCKED)
