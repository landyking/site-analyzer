...
from app.models import ConstraintFactor, MapTaskStatus, SuitabilityFactor
...
class TestToMapTask:
    ...
    def test_to_map_task_success_with_user(self):
        ...
        with patch("app.api.routes._mappers.crud") as mock_crud:
            ...
            assert result.started_at == datetime(2023, 1, 15, 10, 0, 0, tzinfo=UTC)
            assert result.ended_at == datetime(2023, 1, 15, 11, 0, 0, tzinfo=UTC)
            assert result.created_at == datetime(2023, 1, 15, 9, 0, 0, tzinfo=UTC)
...
class TestToMapTaskDetails:
    ...
    def test_to_map_task_details_success(self):
        ...
        mock_data.suitability_factors = (
            '[{"kind": "slope", "weight": 0.3, "breakpoints": [10.0, 20.0], "points": [1, 2, 3]}]'
        )
        ...
        with (
            patch("app.api.routes._mappers.crud") as mock_crud,
...
class TestEdgeCasesAndIntegration:
    ...
    def test_datetime_conversion_edge_cases(self):
        ...
        assert result == datetime(2100, 12, 31, 23, 59, 59, tzinfo=UTC)

        # Test with microseconds
        micro = datetime(2023, 1, 15, 10, 30, 45, 123456)
        result = as_aware_utc(micro)
        assert result == datetime(2023, 1, 15, 10, 30, 45, 123456, tzinfo=UTC)
