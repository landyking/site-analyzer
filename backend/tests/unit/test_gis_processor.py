...
class TestMapTaskMonitor:
    ...
    def test_record_file_success(
        self, mock_logger, mock_engine, mock_session_class, mock_save_task_file
    ):
    ...
    @patch.object(processor_module, "db_engine")
    @patch.object(processor_module, "logger")
    def test_record_file_database_error(
        self, mock_logger, mock_engine, mock_session_class, mock_save_task_file
    ):
...
class TestHelperFunctions:
    ...
    def test_load_task_success(self, mock_engine, mock_session_class):
        ...
        mock_task = MapTaskDB(
            id=123, user_id=456, name="Test Task", district="001", status=MapTaskStatus.PENDING
        )
        mock_session.exec.return_value.first.return_value = mock_task
    ...
    def test_build_ranges_single_breakpoint(self):
        ...
        expected = [(-float("inf"), 15.0, 2), (15.0, float("inf"), 8)]
        assert result == expected
...
class TestProcessMapTask:
    ...
    def test_process_map_task_success(
        self,
        mock_mkdir,
        mock_logger,
        mock_settings,
        ...
        mock_task.suitability_factors = (
            '[{"kind": "slope", "weight": 1.5, "breakpoints": [5, 15], "points": [10, 5, 2]}]'
        )
    ...
    @patch.object(processor_module, "_load_task")
    @patch.object(processor_module, "logger")
    def test_process_map_task_cancelled_during_processing(
        self,
        mock_logger,
        mock_load_task,
        mock_quick_update,
    ...
    def test_process_map_task_engine_failure(
        self,
        mock_mkdir,
        mock_logger,
        mock_settings,
    ...
    @patch.object(processor_module, "logger")
    @patch.object(processor_module.Path, "mkdir")  # Mock directory creation
    def test_process_map_task_invalid_json_factors(
        self,
        mock_mkdir,
        mock_logger,
        mock_settings,
    ...
    @patch.object(processor_module.os.path, "exists")
    @patch.object(processor_module, "logger")
    def test_process_map_task_cleanup_directory_missing(
        self, mock_logger, mock_exists, mock_rmtree
    ):
        """Test cleanup when output directory doesn't exist."""
        # Arrange
        with (
            patch.object(processor_module, "_load_task") as mock_load_task,
    ...
    def test_process_map_task_cleanup_error(
        self, mock_mkdir, mock_settings, mock_logger, mock_exists, mock_rmtree
    ):
