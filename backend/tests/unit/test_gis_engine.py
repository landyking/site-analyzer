...
class MockTaskMonitor:
    ...
    def record_error(self, error_msg: str, phase: str = None, percent: int = None, description: str = None) -> None:
...
def sample_engine_configs():
    ...
    return EngineConfigs(
        ...
        suitability_factors=[
            EngineSuitabilityFactor(kind="slope", weight=1.5, ranges=[(0, 5, 10), (5, 10, 8), (10, 15, 5), (15, 90, 2)]),
            EngineSuitabilityFactor(kind="roads", weight=1.5, ranges=[(0, 1000, 10), (1000, 2000, 8), (2000, 3000, 5), (3000, float('inf'), 2)]),
            EngineSuitabilityFactor(kind="solar", weight=4.0, ranges=[(115, 125, 2), (125, 135, 4), (135, 140, 6), (140, 145, 8), (145, 150, 9), (150, 155, 10)])
        ]
    )
...
class TestSiteSuitabilityEngineInitialization:
    ...
    @patch('os.makedirs')
    @patch('os.path.join')
    def test_init_creates_directories(self, mock_join, mock_makedirs, temp_dirs, sample_engine_configs):
    ...
    def test_factor_initialization_restricted_only(self, temp_dirs):
        ...
        configs = EngineConfigs(
            restricted_factors=[
                EngineRestrictedFactor(kind="rivers", buffer_distance=300)
            ],
            suitability_factors=[]
        )
    ...
    def test_factor_initialization_suitability_only(self, temp_dirs):
        ...
        configs = EngineConfigs(
            restricted_factors=[],
            suitability_factors=[
                EngineSuitabilityFactor(kind="slope", weight=2.0)
            ]
        )
...
class TestSiteSuitabilityEngineDataProcessing:
    ...
    def test_clip_data_shapefile(self, mock_logger, mock_clip, engine):
        ...
        result = engine._clip_data(factor, prepared_data, district_name, district_boundary_shp, district_boundary_bbox_shp)
    ...
    def test_create_restricted_area(self, mock_logger, mock_clip, mock_buffer, engine):
        ...
        result = engine._create_restricted_area(factor, prepared_data, district_name, district_boundary_shp, district_boundary_bbox_shp)
...
class TestSiteSuitabilityEngineEvaluation:
    ...
    @patch('app.gis.engine.tools.get_data_range')
    def test_evaluate_distance_vector(self, mock_get_range, mock_reclassify, mock_distance, mock_poly_to_raster, engine):
        """Test evaluating distance to vector features."""
        factor = {
            'name': 'roads',
            'evaluate_rules': [(0, 1000, 10), (1000, 2000, 8)]
        }
        ...
        result = engine._evaluate_distance_vector(factor, prepared_data, district_name, district_boundary)
    ...
    def test_evaluate_radiation(self, mock_get_range, mock_reclassify, engine):
        ...
        factor = {
            'name': 'solar',
            'evaluate_rules': [(115, 125, 2), (125, 135, 4)]
        }
    ...
    def test_evaluate_temperature(self, mock_get_range, mock_reclassify, engine):
        ...
        result = engine._evaluate_temperature(factor, prepared_data, district_name, district_boundary)
...
class TestSiteSuitabilityEngineRasterTemplate:
    ...
    def test_raster_template_create_default(self, mock_logger, mock_extract, mock_exists, engine):
        ...
        mock_extract.assert_called_once_with(engine.in_solar_radiation, district_boundary_shp, result)
        mock_logger.info.assert_called_with("No suitable raster found for template; using solar radiation raster by default")
...
class TestSiteSuitabilityEngineProcessDistrict:
    ...
    @patch('app.gis.engine.logger')
    def test_process_district_preparation_phase(self, mock_logger, mock_gen_bbox, mock_select, engine):
    ...
    def test_process_district_full_workflow(self, mock_apply_mask, mock_combine, mock_poly_to_raster, 
                                          mock_union, mock_gen_bbox, mock_select, engine):
        ...
        for i, factor in enumerate(engine.factors):
            ...
            if i < 3:  # First 3 are restricted factors
                factor['method_restricted_zone'] = MagicMock(return_value=f"restricted_{factor['name']}.shp")
            else:
                factor['method_restricted_zone'] = MagicMock(return_value=None)
    ...
    def test_process_district_no_restricted_zones(self, engine):
        ...
        with patch('app.gis.engine.RPL_Select_analysis'), \
             patch('app.gis.engine.gen_bounding_box'), \
...
class TestSiteSuitabilityEngineRun:
    ...
    def test_run_cancelled_during_processing(self, engine):
        ...
        with patch.object(engine, 'process_district', side_effect=cancel_after_first) as mock_process:
            results = engine.run(selected_districts=['001', '002'], monitor=monitor)
        ...
        return EngineConfigs(
            ...
            suitability_factors=[
                ...
                EngineSuitabilityFactor(
                    kind="roads", 
                    weight=2.0,
                    ranges=[(0, 500, 10), (500, 1000, 8), (1000, 2000, 6), (2000, 5000, 3), (5000, float('inf'), 1)]
                )
            ]
        )
