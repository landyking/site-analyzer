...
class TestTouchLastLogin:
    ...
    def test_touch_last_login_with_existing_timestamp(self):
        ...
        # Arrange
        mock_session = Mock(spec=Session)
        old_timestamp = datetime(2022, 12, 31, 23, 59, 59, tzinfo=UTC)
        user = UserDB(
            id=1,
            email="test@example.com",
            password_hash="hashed",
...
class TestCreateUser:
    ...
    def test_create_user_success(self, mock_hash, mock_settings):
        ...
        user_create = UserCreate(
            email="test@example.com", password="password123", provider="local", sub="test-user-sub"
        )
...
class TestGetFilesById:
    ...
    def test_get_files_by_id_found(self):
        ...
        expected_files = [
            MapTaskFileDB(
                id=1, user_id=1, map_task_id=1, file_type="result", file_path="/path/to/file1"
            ),
            MapTaskFileDB(
                id=2, user_id=1, map_task_id=1, file_type="log", file_path="/path/to/file2"
            ),
        ]
...
class TestGetFileByConditions:
    ...
    def test_get_file_by_conditions_not_found(self):
        ...
        # Act
        result = get_file_by_conditions(
            session=mock_session, map_task_id=999, file_type="nonexistent"
        )
...
class TestAuthenticate:
    ...
    def test_authenticate_success(self, mock_touch, mock_verify, mock_get_user):
        ...
        # Act
        result = authenticate(
            session=mock_session, email="test@example.com", password="password123"
        )
    ...
    def test_authenticate_user_not_found(self, mock_get_user):
        ...
        # Act
        result = authenticate(
            session=mock_session, email="nonexistent@example.com", password="password123"
        )
    ...
    def test_authenticate_wrong_password(self, mock_verify, mock_get_user):
        ...
        # Act
        result = authenticate(
            session=mock_session, email="test@example.com", password="wrongpassword"
        )
...
class TestCreateMapTask:
    ...
    def test_create_map_task_success(self, mock_process, mock_settings):
        ...
        suitability_factors = [
            SuitabilityFactor(kind="slope", weight=0.5, breakpoints=[0, 10], points=[0, 5, 10])
        ]
        ...
        result = create_map_task(
            session=mock_session, user_id=1, payload=payload, background_tasks=mock_background_tasks
        )
    ...
    def test_create_map_task_read_only_mode(self, mock_settings):
        ...
        payload = CreateMapTaskReq(
            ...
            constraint_factors=[],
            suitability_factors=[
                SuitabilityFactor(kind="slope", weight=1.0, breakpoints=[0, 10], points=[0, 5, 10])
            ],
        )
    ...
    def test_create_map_task_no_background_tasks(self, mock_settings):
        ...
        with patch("builtins.print") as mock_print:
            result = create_map_task(
                session=mock_session, user_id=1, payload=payload, background_tasks=None
            )
...
class TestGetMapTask:
    ...
    def test_get_map_task_found(self):
        ...
        # Arrange
        mock_session = Mock(spec=Session)
        expected_task = MapTaskDB(id=1, user_id=1, district=1, status=MapTaskStatus.PENDING)
